...
input_tensor = vgg_model.input #tensor object
...
def build_style_matrix(vgg_feature_tensor):
    perm_feature_tensor = K.permute_dimensions(vgg_feature_tensor, (0, 3, 1, 2))
    ...
    flattened_feature_tensor = K.reshape(perm_feature_tensor, (-1, num_channels, height * width))

    transposed_feature_tensor = K.permute_dimensions(flattened_feature_tensor, (0, 2, 1))
    style_matrix = (K.batch_dot(flattened_feature_tensor, transposed_feature_tensor) /
                    (num_channels * height * width)
                    )

    return style_matrix

# keras layer is like a reusable component
# a layer knows how to turn an input into an output
...
#
# content_value, *style_values = featurization_model.predict(np.expand_dims(im_data, axis=0))
# print(content_value.shape)
